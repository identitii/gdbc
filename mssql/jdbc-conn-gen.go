...
/*
#cgo CFLAGS: -I.
#cgo LDFLAGS: -L. -lgdbc-mssql
#include <stdlib.h>
#include <./libgdbc-mssql.h>

// TODO: What are isolates? How do they? WHat do? Huh?
graal_isolatethread_t* mssqlCreateIsolate() {
...
int mssqlDestroyIsolate(graal_isolatethread_t* thread) {
...
func openJdbcConn(url, user, password string, txIsolation gdbc.TransactionIsolation) (*jdbcConn, error) {
	...
	var isolate = C.mssqlCreateIsolate()
...
func (c *jdbcConn) Close(keepIsolate bool) error {
	...
	if !keepIsolate {
		defer func() {
			ret := int(C.mssqlDestroyIsolate(c.isolate))
			if ret != 0 {
