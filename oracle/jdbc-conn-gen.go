...
int oracleDestroyIsolate(graal_isolatethread_t* thread) {
...
func openJdbcConn(url, user, password string, txIsolation gdbc.TransactionIsolation) (*jdbcConn, error) {
	...
	var isolate = C.oracleCreateIsolate()
...
func (c *jdbcConn) Close(keepIsolate bool) error {
	...
	if !keepIsolate {
		defer func() {
			ret := int(C.oracleDestroyIsolate(c.isolate))
			if ret != 0 {
